...
export const authorizationHeader = (authorization: string): HeadersInit => {
  return { 'X-Authorization': `Bearer ${authorization}` }
}

export const http = {
  get: (path: string, header: HeadersInit, query: object = {}, prefixPath: string = API_PATH) =>
    send('GET', withQuery(path, encodeKVs(query)), header, prefixPath)
      .then(resWithJson)
      .catch(serviceUnavailable),
  post: (path: string, header: HeadersInit, body: object, prefixPath: string = API_PATH) =>
    sendJson('POST', path, body, { ...jsonHeader, ...header }, prefixPath)
      .then(resWithJson)
      .catch(serviceUnavailable),
  put: (path: string, header: HeadersInit, body: object, prefixPath: string = API_PATH, sessionKey?: string) =>
  ...
  putFile: (path: string, headers: HeadersInit, file: File) =>
    sendFile('PUT', path, headers, file)
      ...
      .catch(serviceUnavailable),
  delete: (path: string, header: HeadersInit, query: object = {}, prefixPath: string = API_PATH, sessionKey?: string) =>
